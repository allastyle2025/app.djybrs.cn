...
class CurrentCheckInsPage extends StatefulWidget {
  ...
  @override
  State<CurrentCheckInsPage> createState() => CurrentCheckInsPageState();
}

class CurrentCheckInsPageState extends State<CurrentCheckInsPage> {
  ...
  // 分类
  final List<String> _pendingCategory = ['待审'];
  final List<String> _otherCategories = ['义工', '学修', '常住', '师父', '其它'];
  ...
  /// 外部可调用的刷新方法，Dashboard 可以通过 GlobalKey 触发
                    ...
                    ? Row(
                        children: [
                          ...
                          NavigationRail(
                            ...
                            onDestinationSelected: (index) => _filterByCategory(allCategories[index]),
                            ...
                            destinations: allCategories.map((category) {
                              final count = _getCategoryCount(category);
                              return NavigationRailDestination(
                                icon: _buildCategoryIcon(category),
  ...
  Widget _buildCategoryIcon(String category) {
    switch (category) {
      case '待审':
        return const Icon(Icons.pending_actions, color: Colors.red);
      case '义工':
        return const Icon(Icons.volunteer_activism, color: Colors.orange);
      case '学修':
  ...
  Widget _buildPendingListView() {
    final pendingCheckIns = _filteredCheckIns.where((checkIn) {
      return checkIn.status == 'PENDING';
    }).toList();
    
    // 按入住时间倒序排序
    final sortedCheckIns = List<RoomCheckIn>.from(pendingCheckIns)
      ..sort((a, b) => b.checkInTime.compareTo(a.checkInTime));
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          final checkIn = sortedCheckIns[index];
          return _buildPendingCheckInItem(checkIn);
        },
      ),
    );
  }

  Widget _buildPendingCheckInItem(RoomCheckIn checkIn) {
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                ...
                                Text(
                                  checkIn.cphone,
                                  style: TextStyle(fontSize: 13, color: RoomColors.textSecondary),
                                ),
                              ],
                            ),
                          ],
                    ...
                    children: [
                      Container(
                        ...
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Icon(Icons.meeting_room_outlined, size: 12, color: RoomColors.textSecondary),
                            const SizedBox(width: 2),
                            Text(
                      ...
                      if (checkIn.remark != null && checkIn.remark!.isNotEmpty) ...[
                        ...
                        Container(
                          ...
                          child: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              Icon(Icons.note_outlined, size: 11, color: Colors.orange.shade600),
                              const SizedBox(width: 2),
                              Text(
                                '注',
                                style: TextStyle(
  ...
  Color _getPurposeColor(String purpose) {
    switch (purpose) {
      case 'volunteer':
        return Colors.orange; // 义工使用橙色，和待审核一样
      case 'study':
